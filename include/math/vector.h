...
/**
 * @brief Build a 4x4 translation matrix with translation components in the last
 * column.
 * @return basematrix<T,4,4> translation matrix
 */
template <typename T> basematrix<T, 4, 4> translation_matrix(T x, T y, T z) {
    basematrix<T, 4, 4> m;
    m.loadIdentity();
    m[3] = x;
    m[7] = y;
...
/**
 * @brief Apply translation to an existing 4x4 matrix by adding v
 * (post-multiplying the translation column). The function updates the last
 * column entries to include the transformed translation.
 */
template <typename T>
 ...
 */
template <typename T> basematrix<T, 4, 4> scaling_matrix(T x, T y, T z) {
    basematrix<T, 4, 4> m;
    m.loadIdentity();
    m[0] = x;
    m[5] = y;
