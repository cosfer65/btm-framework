...
namespace btm {
template <typename T> struct TessellatedMesh {
...
template <typename T> struct curvatureData {
...
template <typename T> struct meshVertex {
    size_t id;
...
template <typename T> class mesh {
  ...
  public:
    ...
    void addVertex(meshVertex<T> *vertex) {
        vertices[vertex->id] = const_cast<meshVertex<T> *>(vertex);
    }

    meshVertex<T> *addVertex(size_t id, const basevec3<T> &position) {
    ...
    void addFace(size_t v1, size_t v2, size_t v3) {
        bool flip = checkFlipEdge(v1, v2) || checkFlipEdge(v2, v3) ||
                    checkFlipEdge(v3, v1);
        if (flip) {
            size_t t = v2;
            v2 = v3;
    ...
    void addFace(size_t v1, size_t v2, size_t v3, size_t v4) {
        bool flip = checkFlipEdge(v1, v2) || checkFlipEdge(v2, v3) ||
                    checkFlipEdge(v3, v4) || checkFlipEdge(v4, v1);
        if (flip) {
            size_t t = v2;
            v2 = v4;
            v4 = t;
...
template <typename T> mesh<T> *parse_mesh(const TessellatedMesh<T> &_mesh_def) {
    ...
    for (size_t idx = 0; idx < index_count; idx += 3) {
        _mesh->addFace(_mesh_def.indices[idx], _mesh_def.indices[idx + 1],
                       _mesh_def.indices[idx + 2]);
    }
...
template <typename T> mesh<T> *parse_mesh(const mesh_def<T> &_mesh_def) {
    ...
    for (const std::tuple<size_t, T, T, T> &vert : _mesh_def.vertices) {
        _mesh->addVertex(std::get<0>(vert),
