    ...
    public:
        ...
        void build_attributes() {
            for (auto& tria : triangles) {
                const auto& v0 = vertices[tria.v0].position;
                const auto& v1 = vertices[tria.v1].position;
                const auto& v2 = vertices[tria.v2].position;
        ...
        // some functions to add vertices and triangles are added here for
        ...
        // when adding data to the mesh. For example, when adding a triangle, we
        // should ensure that the vertex indices are valid and that the adjacency
        // information is updated accordingly.
        std::uint32_t add_vertex(const basevec3<T>& position) {
        ...
        std::uint32_t add_triangle(std::uint32_t v0, std::uint32_t v1,
            std::uint32_t v2) {
            return add_triangle({ v0, v1, v2 });
        }

        std::uint32_t triangle_count() const { return triangles.size(); }
