...
namespace btm {
/**
 * @brief Calculates the unit normal vector of a plane defined by three points
 ...
 */
template <typename T> T tangent(T a) {
 ...
 */
template <typename T>
T vector_angle(const basevec3<T> &edge1, const basevec3<T> &edge2) {
 ...
 * c.
 *
 * This function interprets the input points as forming two vectors ba = b - a'
 * and bc = c - a' that share the common vertex at 'a'. It then computes the
 * cotangent of the angle between these two vectors using the relation:
 *
 *     cot(theta) = (ba * bc) / ||ba ? bc||
 *
 * A small cross-product length (below 'TOLLERANCE<T>') is treated as a
 * degenerate case and results in a return value of 'T(0)'.
 *
 ...
 * @return Cotangent of the angle at point 'a', or 'T(0)' for degenerate
 * configurations.
 */
template <typename T>
T cotangent(const basevec3<T> &a, const basevec3<T> &b, const basevec3<T> &c) {
    basevec3<T> ba = b - a;
    basevec3<T> bc = c - a;
    basevec3<T> cross = ba.cross(bc);
...
/**
 * @brief Computes the interior angle at vertex pa for the triangle (pa, pb,
 * pc).
 *
 * The function constructs two edge vectors 'u = pb - pa' and 'v = pc - pa',
 * both originating from the vertex 'pa'. It then computes the angle between
 * them using the standard dot-product relation:
 *
 *     cos(theta) = (u * v) / (||u|| * ||v||)
 *
 * The cosine value is clamped to the range [-1, 1] to improve numerical
 * robustness before applying 'std::acos'. If either edge vector has zero
 * length, the configuration is treated as degenerate and 'T(0)' is returned.
 *
 * @tparam T Numeric type used for coordinates and computations.
 ...
 * @return Corner angle at 'pa' in radians, or 'T(0)' for degenerate
 * configurations.
 */
template <typename T>
 ...
 *
 * Given two vector pairs '(u_f, v_f)' and '(u_p, v_p)', this function checks if
 * the planes spanned by each pair are the same (or nearly the same) by
 * comparing their normals. It computes:
 *
 *   - 'cross1 = u_f * v_f' : normal to the first plane
 *   - 'cross2 = u_p * v_p' : normal to the second plane
 *   - 'cross_res = cross1 * cross2'
 *
 * If 'cross_res' has near-zero length (less than 'TOLLERANCE<T>'), the two
 * normals are considered parallel or anti-parallel, and the vector pairs are
 * treated as coplanar.
 *
 * @tparam T Numeric type used for coordinates and computations.
 ...
 * @return 'true' if the vector pairs are coplanar within tolerance, otherwise
 * 'false'.
 */
template <typename T>
...
/**
 * @brief Constructs a local orthonormal tangent frame (u, v) for a given
 * normal.
 *
 * This function builds two tangent vectors `u` and `v` that are:
 *   - orthogonal to the input `normal` (i.e., lie in the plane perpendicular to
 * it),
 *   - mutually orthogonal to each other (forming a right-handed local frame
 * when combined with `normal`),
 ...
 *     (its length must be at least `TOLLERANCE<float>`); otherwise it returns
 * `false`.
...
bool create_uv_reference_plane(const basevec3<T> &normal, basevec3<T> &u,
    ...
    // General case
        ...
        u = basevec3<T>(normal.y(), -normal.x(), 0);
        v = basevec3<T>(normal.x() * normal.z(), normal.y() * normal.z(),
